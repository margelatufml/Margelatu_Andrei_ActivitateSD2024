...
struct MagazinAutomobile* copiazavector(int n, struct MagazinAutomobile* vector) {
...
int main() {
    ...
    struct MagazinAutomobile* vector;
    vector = (struct MagazinAutomobile*)malloc(5 * (sizeof(struct MagazinAutomobile) + 1));
    printf("Citire Magazine pentru Automobile\n");
    for (int i = 0; i < 5; i++)
    {
        printf("Magazinul %d\n", i + 1);
        citesteAutomobilMagazin(&vector[i]);
    }
